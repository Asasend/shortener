...
var (
	...
	cacheDb3SequenceIdPrefix   = "cache:db3:sequence:id:"
	cacheDb3SequenceStubPrefix = "cache:db3:sequence:stub:"
)
...
func (m *defaultSequenceModel) Delete(ctx context.Context, id uint64) error {
	...
	db3SequenceIdKey := fmt.Sprintf("%s%v", cacheDb3SequenceIdPrefix, id)
	db3SequenceStubKey := fmt.Sprintf("%s%v", cacheDb3SequenceStubPrefix, data.Stub)
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, db3SequenceIdKey, db3SequenceStubKey)
	return err
}
...
func (m *defaultSequenceModel) FindOne(ctx context.Context, id uint64) (*Sequence, error) {
	db3SequenceIdKey := fmt.Sprintf("%s%v", cacheDb3SequenceIdPrefix, id)
	var resp Sequence
	err := m.QueryRowCtx(ctx, &resp, db3SequenceIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultSequenceModel) FindOneByStub(ctx context.Context, stub string) (*Sequence, error) {
	db3SequenceStubKey := fmt.Sprintf("%s%v", cacheDb3SequenceStubPrefix, stub)
	var resp Sequence
	err := m.QueryRowIndexCtx(ctx, &resp, db3SequenceStubKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
...
func (m *defaultSequenceModel) Insert(ctx context.Context, data *Sequence) (sql.Result, error) {
	db3SequenceIdKey := fmt.Sprintf("%s%v", cacheDb3SequenceIdPrefix, data.Id)
	db3SequenceStubKey := fmt.Sprintf("%s%v", cacheDb3SequenceStubPrefix, data.Stub)
...
func (m *defaultSequenceModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheDb3SequenceIdPrefix, primary)
}
